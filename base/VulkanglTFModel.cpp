...
vkglTF::PointModel::~PointModel()
{
	vkDestroyBuffer(device->logicalDevice, vertices.vertert.buffer, nullptr);
	vkFreeMemory(device->logicalDevice, vertices.vertert.memory, nullptr);
	vkDestroyBuffer(device->logicalDevice, aabbs.aabb.buffer, nullptr);
	vkFreeMemory(device->logicalDevice, aabbs.aabb.memory, nullptr);
	if (descriptorSetLayoutUbo != VK_NULL_HANDLE) {
		vkDestroyDescriptorSetLayout(device->logicalDevice, descriptorSetLayoutUbo, nullptr);
		descriptorSetLayoutUbo = VK_NULL_HANDLE;
	}
...
void vkglTF::PointModel::loadPointCloud(std::vector<Vertert>&points, std::vector<Dimensions>&AABBs, vks::VulkanDevice *device, VkQueue transferQueue) {
	this->device = device;

	//allocata layout Sets
	size_t pointBufferSize = points.size() * sizeof(Vertert);
	size_t aabbBufferSize = AABBs.size() * sizeof(Dimensions);

	vertices.count = static_cast<uint32_t>(points.size());
	aabbs.count = static_cast<uint32_t>(AABBs.size());

	vks::Buffer vertexStaging, aabbStaging;
	...
	// Vertex buffer
	VK_CHECK_RESULT(device->createBuffer(
		VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT | memoryPropertyFlags | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
		pointBufferSize,
		&vertices.vertert.buffer,
		&vertices.vertert.memory));
	// aabb buffer
	VK_CHECK_RESULT(device->createBuffer(
		VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT | memoryPropertyFlags | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
		aabbBufferSize,
		&aabbs.aabb.buffer,
		&aabbs.aabb.memory));
	...
	vkCmdCopyBuffer(copyCmd, vertexStaging.buffer, vertices.vertert.buffer, 1, &copyRegion);

	copyRegion.size = aabbBufferSize;
	vkCmdCopyBuffer(copyCmd, aabbStaging.buffer, aabbs.aabb.buffer, 1, &copyRegion);
...
void vkglTF::PointModel::draw(VkCommandBuffer commandBuffer, uint32_t renderFlags , VkPipeline pipeline , uint32_t bindImageSet ) {
	if (vertices.vertert.buffer == VK_NULL_HANDLE) {
		return;
	}
	VkDeviceSize offsets[] = { 0 };
	vkCmdBindVertexBuffers(commandBuffer, 0, 1, &vertices.vertert.buffer, offsets);
