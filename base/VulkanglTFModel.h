...
namespace vkglTF
{
	...
	*/
	...
	struct PointModel {
		...
		~PointModel();
		int numpoint;
		void loadPointCloud(std::vector<Vertert>&points, std::vector<Dimensions>&AABBs, vks::VulkanDevice *device, VkQueue transferQueue);
		void draw(VkCommandBuffer commandBuffer, uint32_t renderFlags = 0, VkPipeline pipeline = VK_NULL_HANDLE, uint32_t bindImageSet = 1);
	};
}